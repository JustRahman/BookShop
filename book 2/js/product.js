api_products = [{
    ...
    },
    {
        ...
        "price": 149.9,
        "body": `An engrossing, insider’s account of how a teacher built one of the world’s most valuable companies—rivaling Walmart & Amazon—and forever reshaped the global economy.<br>In just a decade and half Jack Ma, a man from modest beginnings who started out as an English teacher, founded and built Alibaba into one of the world’s largest companies, an e-commerce empire on which hundreds of millions of Chinese consumers depend. Alibaba’s $25 billion IPO in 2014 was the largest global IPO ever. A Rockefeller of his age who is courted by CEOs and Presidents around the world, Jack is an icon for China’s booming private sector and the gatekeeper to hundreds of millions of middle class consumers.<br>Duncan Clark first met Jack in 1999 in the small apartment where Jack founded Alibaba. Granted unprecedented access to a wealth of new material including exclusive interviews, Clark draws on his own experience as an early advisor to Alibaba and two decades in China chronicling the Internet’s impact on the country to create an authoritative, compelling narrative account of Alibaba’s rise.<br>How did Jack overcome his humble origins and early failures to achieve massive success with Alibaba? How did he outsmart rival entrepreneurs from China and Silicon Valley? Can Alibaba maintain its 80% market share? As it forges ahead into finance and entertainment, are there limits to Alibaba’s ambitions?  How does the Chinese government view its rise?  Will Alibaba expand further overseas, including in the U.S.?<br>Clark tells Alibaba’s tale in the context of China’s momentous economic and social changes, illuminating an unlikely corporate titan as never before.`,
    },
    {
        "id": 1650,
...
var showedProductId = api_products[0]['id'];

function updateProduct() {
    let index = api_products.findIndex(obj => obj.id === showedProductId);

    document.getElementById('product').innerHTML = `<div class="row my-2 my-sm-3">
        <div class="col-sm-5"><img src="img/` + api_products[index].image + `" alt="" class="img-fluid border rounded"></div>
        <div class="col-sm-7">
            <div class="h4 text-dark">` + api_products[index].name + `</div>
            <div class="h5 text-primary font-weight-bold">` + (api_products[index].price).toFixed(2) + ` <small>TMT</small></div>
            <div>` + api_products[index].body + `</div>
        </div>
    </div>`;
    ...
    for (let i = 0; i < api_products.length; i++) {
        if (api_products[i].id !== showedProductId) {
            products.innerHTML += `<div class="row no-gutters my-2 my-sm-3">
                <div class="col-4">
                    <a href="#" class="text-decoration-none" onclick="showProduct(` + api_products[i].id + `)">
                        <img src="img/sm/` + api_products[i].image + `" alt="" class="img-fluid border rounded">
                    </a>
                </div>
                <div class="col-8 pl-2 pl-sm-3">
                    <a href="#" class="h6 text-dark text-decoration-none" onclick="showProduct(` + api_products[i].id + `)">` + api_products[i].name + `</a>
                    <div class="text-primary font-weight-bold">` + (api_products[i].price).toFixed(2) + ` <small>TMT</small></div>
                </div>
            </div>`;
        }
    }
